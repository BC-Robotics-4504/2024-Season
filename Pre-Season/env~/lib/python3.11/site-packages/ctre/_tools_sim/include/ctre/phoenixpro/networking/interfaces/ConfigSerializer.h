...
extern "C"
{
    ...
    CTREXPORT int c_ctre_phoenixpro_serialize_double_no_malloc(int spn, double value, char *str, uint32_t max_len);
    /* Note: This returns a malloc'd char array that caller needs to free */
    CTREXPORT int c_ctre_phoenixpro_serialize_int(int spn, int value, char **str);
    CTREXPORT int c_ctre_phoenixpro_serialize_int_no_malloc(int spn, int value, char *str, uint32_t max_len);
    /* Note: This returns a malloc'd char array that caller needs to free */
    CTREXPORT int c_ctre_phoenixpro_serialize_bool(int spn, bool value, char **str);
    CTREXPORT int c_ctre_phoenixpro_serialize_bool_no_malloc(int spn, bool value, char *str, uint32_t max_len);
    /* Note: This returns a malloc'd char array that caller needs to free */
    CTREXPORT int c_ctre_phoenixpro_serialize_pgn(int spn, uint16_t frame_index, uint16_t framePeriodMs, char **str);
    CTREXPORT int c_ctre_phoenixpro_serialize_pgn_no_malloc(int spn, uint16_t frame_index, uint16_t framePeriodMs, char *str, uint32_t max_len);
    ...
    CTREXPORT int c_ctre_phoenixpro_deserialize_pgn(int spn, const char *str, uint32_t strlen, uint16_t *frame_index, uint16_t *framePeriodMs);
#ifdef __cplusplus
}
#endif